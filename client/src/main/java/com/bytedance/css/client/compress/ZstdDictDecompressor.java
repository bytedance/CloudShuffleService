 ...
 */
public class ZstdDictDecompressor implements Decompressor {

  private static final Logger logger = Logger.getLogger(ZstdDictDecompressor.class);

  private final Checksum checksum;
  private final ChildFirstURLClassLoader classLoader;
  private final Method decompressUsingDictMethod;

  private volatile byte[] dict;

  public ZstdDictDecompressor(byte[] dict) {
    try {
      ...
      decompressUsingDictMethod = zstdClass.getDeclaredMethod("decompressUsingDict",
    ...
    try {
      ...
      switch (compressionMethod) {
        ...
        case COMPRESSION_METHOD_CSS:
          long tmp = (long) decompressUsingDictMethod.invoke(
            null, dst, dstOff, src, HEADER_LENGTH, compressedLen, dict);
