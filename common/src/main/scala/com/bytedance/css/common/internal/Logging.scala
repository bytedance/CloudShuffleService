...
package com.bytedance.css.common.internal
  ...
  private def initializeLogging(isInterpreter: Boolean, silent: Boolean): Unit = {
    ...
    if (Logging.isLog4j12()) {
      ...
      if (!log4j12Initialized) {
        ...
        var defaultLogProps = Utils.getClassLoader.getResource("log4j-defaults.properties")
        val loggProps = sys.env.get("CSS_CONF_DIR")
          .orElse(sys.env.get("CSS_HOME").map { t => s"$t${File.separator}conf" })
          .map { t => new File(s"$t${File.separator}log4j.properties") }
          .filter(_.isFile)
          .map(_.getAbsolutePath)
          .getOrElse(null)
        if (loggProps != null && new java.io.File(loggProps).exists() && new java.io.File(loggProps).canRead) {
          defaultLogProps = new java.io.File(loggProps).toURL
        }
        Option(defaultLogProps) match {
          case Some(url) =>
            PropertyConfigurator.configure(url)
            if (!silent) {
              System.err.println(s"Using Css's default log4j profile: $defaultLogProps")
            }
          case None =>
            System.err.println(s"Css was unable to load $defaultLogProps")
        }
      }
