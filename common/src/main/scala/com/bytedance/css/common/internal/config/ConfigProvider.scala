...
package com.bytedance.css.common.internal.config
 ...
 */
class CssConfigProvider(conf: JMap[String, String]) extends ConfigProvider {

  override def get(key: String): Option[String] = {
    if (key.startsWith("css.")) {
      Option(conf.get(key)).orElse(CssConf.getDeprecatedConfig(key, conf))
    } else {
      None
    }
  }
