...
class StatsdReporter(
  ...
  override def report(
    ...
    Try(new DatagramSocket) match {
      case Failure(ioe: IOException) => logWarning(s"StatsD datagram socket construction failed" +
        s" host ${host} port ${port} localhost ${Utils.localHostName()} port ${0}", ioe)
      ...
      case Success(s) =>
        ...
        Try {
          ...
          case ioe: IOException =>
            logDebug(s"Unable to send packets to StatsD host ${address.getHostString} port ${address.getPort}" +
              s" localhost ${localAddress} port ${localPort}", ioe)
          case e: Throwable => logDebug(s"Unable to send packets to StatsD at '$host:$port'", e)
        }
        Try(socket.close()) recover {
          case ioe: IOException =>
            logDebug("Error when close socket to StatsD host ${address.getHostString} port ${address.getPort}" +
              s" localhost ${localAddress} port ${localPort}", ioe)
