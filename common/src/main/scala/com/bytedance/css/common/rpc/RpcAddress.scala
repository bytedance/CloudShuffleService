...
case class RpcAddress(host: String, port: Int) {
  ...
  /** Returns a string in the form of "css://host:port". */
  def toCssURL: String = "css://" + hostPort
...
object RpcAddress {
  ...
  /** Returns the [[RpcAddress]] encoded in the form of "css://host:port" */
  def fromCssURL(cssUrl: String): RpcAddress = {
    val (host, port) = Utils.extractHostPortFromCssUrl(cssUrl)
