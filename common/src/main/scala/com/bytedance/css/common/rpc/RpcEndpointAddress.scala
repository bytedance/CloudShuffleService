...
case class RpcEndpointAddress(rpcAddress: RpcAddress, name: String) {
  ...
  override val toString = if (rpcAddress != null) {
      s"css://$name@${rpcAddress.host}:${rpcAddress.port}"
    } else {
      s"css-client://$name"
    }
}
...
object RpcEndpointAddress {
  ...
  def apply(cssUrl: String): RpcEndpointAddress = {
    try {
      val uri = new java.net.URI(cssUrl)
      val host = uri.getHost
      val port = uri.getPort
      val name = uri.getUserInfo
      if (uri.getScheme != "css" ||
          host == null ||
          port < 0 ||
          name == null ||
          ...
          uri.getQuery != null) {
        throw new CssException("Invalid Css URL: " + cssUrl)
      }
      new RpcEndpointAddress(host, port, name)
    } catch {
      case e: java.net.URISyntaxException =>
        throw new CssException("Invalid Css URL: " + cssUrl, e)
    }
  }
}
