...
import com.bytedance.css.common.rpc.{RpcEndpoint, RpcEndpointAddress, RpcEndpointRef, RpcEnvStoppedException}
...
class Dispatcher(nettyEnv: NettyRpcEnv, numUsableCores: Int) extends Logging {
  ...
  def postMessage(
    ...
    val error = synchronized {
      ...
      if (stopped) {
        ...
        Some(new CssException(s"Could not find $endpointName."))
      } else {
  ...
  private val threadpool: ThreadPoolExecutor = {
    ...
    val numThreads = nettyEnv.conf.getInt("css.rpc.netty.dispatcher.numThreads",
