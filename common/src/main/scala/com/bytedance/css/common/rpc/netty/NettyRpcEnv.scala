...
import com.bytedance.css.common.rpc.{RpcAddress, RpcEndpoint, RpcEndpointAddress, RpcEndpointNotFoundException}
import com.bytedance.css.common.rpc.{RpcEndpointRef, RpcEnv, RpcEnvConfig, RpcEnvFactory}
import com.bytedance.css.common.rpc.{RpcEnvFileServer, RpcEnvStoppedException, RpcTimeout}
import com.bytedance.css.common.serializer.{JavaSerializer, JavaSerializerInstance, SerializationStream}
import com.bytedance.css.common.util.{ByteBufferInputStream, ByteBufferOutputStream, ThreadUtils, Utils}
import com.bytedance.css.network.TransportContext
import com.bytedance.css.network.client.{RpcResponseCallback, StreamCallback, TransportClient}
import com.bytedance.css.network.client.{TransportClientBootstrap, TransportClientFactory}
import com.bytedance.css.network.server.{NoOpRpcHandler, RpcHandler, StreamManager}
import com.bytedance.css.network.server.{TransportServer, TransportServerBootstrap}
...
class NettyRpcEnv(
  ...
  val transportConf = Utils.fromCssConf(
  ...
  private def downloadClient(host: String, port: Int): TransportClient = {
    if (fileDownloadFactory == null) synchronized {
      if (fileDownloadFactory == null) {
        ...
        val downloadConf = Utils.fromCssConf(clone, module, ioThreads)
...
private[rpc] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    val CssConf = config.conf
    // Use JavaSerializerInstance in multiple threads is safe. However, if we plan to support
    // KryoSerializer in future, we have to use ThreadLocal to store SerializerInstance
    val javaSerializerInstance =
      new JavaSerializer(CssConf).newInstance().asInstanceOf[JavaSerializerInstance]
    val nettyEnv =
      new NettyRpcEnv(CssConf, javaSerializerInstance, config.advertiseAddress,
    ...
    if (!config.clientMode) {
      ...
      try {
        Utils.startServiceOnPort(config.port, startNettyRpcEnv, CssConf, config.name)._1
      } catch {
        case NonFatal(e) =>
          nettyEnv.shutdown()
          throw e
...
class NettyRpcEndpointRef(
    @transient private val conf: CssConf,
