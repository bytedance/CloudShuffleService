...
import com.bytedance.css.common.rpc.RpcEnvFileServer
import com.bytedance.css.common.util.Utils
import com.bytedance.css.network.buffer.{FileSegmentManagedBuffer, ManagedBuffer}
import com.bytedance.css.network.server.StreamManager

class NettyStreamManager(rpcEnv: NettyRpcEnv)
  ...
  override def addFile(file: File): String = {
    ...
    s"${rpcEnv.address.toCssURL}/files/${Utils.encodeFileNameToURIRawPath(file.getName())}"
  }
  ...
  override def addJar(file: File): String = {
    ...
    require(existingPath == null || existingPath == file,
      s"File ${file.getName} was already registered with a different path " +
        s"(old path = $existingPath, new path = $file")
    s"${rpcEnv.address.toCssURL}/jars/${Utils.encodeFileNameToURIRawPath(file.getName())}"
  }
  ...
  override def addDirectory(baseUri: String, path: File): String = {
    ...
    s"${rpcEnv.address.toCssURL}$fixedBaseUri"
  }
