...
import com.bytedance.css.common.rpc.{RpcAddress, RpcEnvStoppedException}
import com.bytedance.css.network.client.{RpcResponseCallback, TransportClient}
...
class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  def send(message: OutboxMessage): Unit = {
    ...
    if (dropped) {
      message.onFailure(new CssException("Message is dropped because Outbox is stopped"))
    } else {
      drainOutbox()
    }
  }
