...
package com.bytedance.css.common.serializer
...
import com.bytedance.css.common.util.{ByteBufferInputStream, ByteBufferOutputStream, Utils}
 ...
 * Spark application.
 */
class JavaSerializer(conf: CssConf) extends Serializer with Externalizable {
  private var counterReset = conf.getInt("css.serializer.objectStreamReset", 100)
  private var extraDebugInfo = conf.getBoolean("css.serializer.extraDebugInfo", true)

  protected def this() = this(new CssConf())  // For deserialization only
