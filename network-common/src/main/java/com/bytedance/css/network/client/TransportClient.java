...
public class TransportClient implements Closeable {
  ...
  public ChannelFuture batchPushData(BatchPushDataRequest batchPushDataRequest, RpcResponseCallback callback) {
    if (logger.isTraceEnabled()) {
      logger.trace("Batch Pushing data to {}", getRemoteAddress(channel));
    }

    long requestId = dataRequestId();
    handler.addRpcRequest(requestId, callback);

    batchPushDataRequest.requestId = requestId;

    RpcChannelListener listener = new RpcChannelListener(requestId, callback);
    return channel.writeAndFlush(batchPushDataRequest).addListener(listener);
  }
