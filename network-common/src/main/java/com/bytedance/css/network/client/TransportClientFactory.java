...
package com.bytedance.css.network.client;
...
import io.netty.buffer.NettyMemoryMetrics;
...
import com.bytedance.css.network.server.TransportChannelHandler;
import com.bytedance.css.network.util.*;
...
public class TransportClientFactory implements Closeable {
  ...
  private NettyMemoryMetrics cssMetrics;
  ...
  public void registerCssMetrics(String cssMetricsPrefix) {
    synchronized (this) {
      if (cssMetrics == null) {
        String cssNettyPrefix = String.format("%s#%s", cssMetricsPrefix, conf.getModuleName() + "-client");
        cssMetrics = new NettyMemoryMetrics(pooledAllocator, cssNettyPrefix, conf);
      }
    }
  }
  ...
  public MetricSet getAllCssMetrics() {
    return cssMetrics;
  }
   ...
   *
   * We maintains an array of clients (size determined by css.shuffle.io.numConnectionsPerPeer)
  ...
  public TransportClient createClient(String remoteHost, int remotePort, int seed)
      throws IOException, InterruptedException {
    return createClient(remoteHost, remotePort, seed, false);
  }

  /**
   * try best to pick the ClientConnection from pool according to the provided seed,
   * if same seed passed in, try to re-use the last connection if possible.
   * @param remoteHost
   * @param remotePort
   * @param seed
   * @return
   ...
   */
  public TransportClient createClient(String remoteHost, int remotePort, int seed, boolean fastFail)
      throws IOException, InterruptedException {
    ...
    int clientIndex = seed < 0 ? rand.nextInt(numConnectionsPerPeer) : seed % numConnectionsPerPeer;
