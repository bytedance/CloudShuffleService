...
import com.bytedance.css.network.protocol.ChunkFetchFailure;
import com.bytedance.css.network.protocol.ChunkFetchRequest;
import com.bytedance.css.network.protocol.ChunkFetchSuccess;
...
public class ChunkFetchRequestHandler extends SimpleChannelInboundHandler<ChunkFetchRequest> {
  ...
  public void processFetchRequest(
    ...
    streamManager.chunkFetchMetrics.getChunkFetchQps().mark();
    Timer.Context latencyTimer = streamManager.chunkFetchMetrics.getChunkFetchLatency().time();
    ...
    try {
      ...
      logger.error(String.format("Error opening block %s for request from %s",
        msg.streamChunkId, NettyUtils.getRemoteAddress(channel)), e);
      streamManager.chunkFetchMetrics.getChunkFetchFailedQPS().mark();
    ...
    respond(channel, new ChunkFetchSuccess(msg.streamChunkId, buf)).addListener(future -> {
      latencyTimer.stop();
      streamManager.chunkFetchMetrics.getChunkFetchThroughput().mark(bufSize);
