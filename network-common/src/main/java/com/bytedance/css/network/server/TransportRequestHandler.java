...
import com.bytedance.css.network.buffer.NioManagedBuffer;
import com.bytedance.css.network.client.*;
import com.bytedance.css.network.protocol.*;
import com.bytedance.css.network.util.TransportFrameDecoder;

import static com.bytedance.css.network.util.NettyUtils.getRemoteAddress;
...
public class TransportRequestHandler extends MessageHandler<RequestMessage> {
  ...
  public void handle(RequestMessage request) throws Exception {
    ...
    } else if (request instanceof BatchPushDataRequest) {
      processBatchPushDataRequest((BatchPushDataRequest) request);
    } else {
  ...
  private void processBatchPushDataRequest(BatchPushDataRequest req) {
    try {
      rpcHandler.receiveBatchPushDataRequest(reverseClient, req, new RpcResponseCallback() {
        @Override
        public void onSuccess(ByteBuffer response) {
          respond(new RpcResponse(req.requestId, new NioManagedBuffer(response)));
        }
        ...
        @Override
        public void onFailure(Throwable e) {
          logger.error("Process batchPushData onFailure!", e);
          respond(new RpcFailure(req.requestId, e.toString()));
        }
      });
    } catch (Exception e) {
      logger.error("Error while invoking RpcHandler#receive() on PushDataRequest " + req, e);
