...
package com.bytedance.css.network.util;
...
public class TransportConf {
  ...
  private final String CSS_NETWORK_IO_MODE_KEY;
  private final String CSS_NETWORK_IO_PREFERDIRECTBUFS_KEY;
  private final String CSS_NETWORK_IO_CONNECTIONTIMEOUT_KEY;
  private final String CSS_NETWORK_IO_BACKLOG_KEY;
  private final String CSS_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY;
  private final String CSS_NETWORK_IO_SERVERTHREADS_KEY;
  private final String CSS_NETWORK_IO_CLIENTTHREADS_KEY;
  private final String CSS_NETWORK_IO_RECEIVEBUFFER_KEY;
  private final String CSS_NETWORK_IO_SENDBUFFER_KEY;
  private final String CSS_NETWORK_SASL_TIMEOUT_KEY;
  private final String CSS_NETWORK_IO_MAXRETRIES_KEY;
  private final String CSS_NETWORK_IO_RETRYWAIT_KEY;
  private final String CSS_NETWORK_IO_LAZYFD_KEY;
  private final String CSS_NETWORK_VERBOSE_METRICS;
  private final String CSS_NETWORK_IO_ENABLETCPKEEPALIVE_KEY;
  ...
  public TransportConf(String module, ConfigProvider conf) {
    this.module = module;
    this.conf = conf;
    CSS_NETWORK_IO_MODE_KEY = getConfKey("io.mode");
    CSS_NETWORK_IO_PREFERDIRECTBUFS_KEY = getConfKey("io.preferDirectBufs");
    CSS_NETWORK_IO_CONNECTIONTIMEOUT_KEY = getConfKey("io.connectionTimeout");
    CSS_NETWORK_IO_BACKLOG_KEY = getConfKey("io.backLog");
    CSS_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY =  getConfKey("io.numConnectionsPerPeer");
    CSS_NETWORK_IO_SERVERTHREADS_KEY = getConfKey("io.serverThreads");
    CSS_NETWORK_IO_CLIENTTHREADS_KEY = getConfKey("io.clientThreads");
    CSS_NETWORK_IO_RECEIVEBUFFER_KEY = getConfKey("io.receiveBuffer");
    CSS_NETWORK_IO_SENDBUFFER_KEY = getConfKey("io.sendBuffer");
    CSS_NETWORK_SASL_TIMEOUT_KEY = getConfKey("sasl.timeout");
    CSS_NETWORK_IO_MAXRETRIES_KEY = getConfKey("io.maxRetries");
    CSS_NETWORK_IO_RETRYWAIT_KEY = getConfKey("io.retryWait");
    CSS_NETWORK_IO_LAZYFD_KEY = getConfKey("io.lazyFD");
    CSS_NETWORK_VERBOSE_METRICS = getConfKey("io.enableVerboseMetrics");
    CSS_NETWORK_IO_ENABLETCPKEEPALIVE_KEY = getConfKey("io.enableTcpKeepAlive");
  }
  ...
  public String ioMode() {
    return conf.get(CSS_NETWORK_IO_MODE_KEY, "NIO").toUpperCase(Locale.ROOT);
  }

  /** If true, we will prefer allocating off-heap byte buffers within Netty. */
  public boolean preferDirectBufs() {
    return conf.getBoolean(CSS_NETWORK_IO_PREFERDIRECTBUFS_KEY, true);
  }

  /** Connect timeout in milliseconds. Default 120 secs. */
  public int connectionTimeoutMs() {
    long defaultNetworkTimeoutS = JavaUtils.timeStringAsSec(
      conf.get("css.network.timeout", "120s"));
    long defaultTimeoutMs = JavaUtils.timeStringAsSec(
      conf.get(CSS_NETWORK_IO_CONNECTIONTIMEOUT_KEY, defaultNetworkTimeoutS + "s")) * 1000;
    return (int) defaultTimeoutMs;
  }

  /** Number of concurrent connections between two nodes for fetching data. */
  public int numConnectionsPerPeer() {
    return conf.getInt(CSS_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY, 1);
  }

  /** Requested maximum length of the queue of incoming connections. Default -1 for no backlog. */
  public int backLog() { return conf.getInt(CSS_NETWORK_IO_BACKLOG_KEY, -1); }

  /** Number of threads used in the server thread pool. Default to 0, which is 2x#cores. */
  public int serverThreads() { return conf.getInt(CSS_NETWORK_IO_SERVERTHREADS_KEY, 0); }

  /** Number of threads used in the client thread pool. Default to 0, which is 2x#cores. */
  public int clientThreads() { return conf.getInt(CSS_NETWORK_IO_CLIENTTHREADS_KEY, 0); }
   ...
   */
  public int receiveBuf() { return conf.getInt(CSS_NETWORK_IO_RECEIVEBUFFER_KEY, -1); }

  /** Send buffer size (SO_SNDBUF). */
  public int sendBuf() { return conf.getInt(CSS_NETWORK_IO_SENDBUFFER_KEY, -1); }

  /** Timeout for a single round trip of auth message exchange, in milliseconds. */
  public int authRTTimeoutMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get("css.network.auth.rpcTimeout",
      conf.get(CSS_NETWORK_SASL_TIMEOUT_KEY, "30s"))) * 1000;
  }
   ...
   */
  public int maxIORetries() { return conf.getInt(CSS_NETWORK_IO_MAXRETRIES_KEY, 3); }
  ...
  public int ioRetryWaitTimeMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get(CSS_NETWORK_IO_RETRYWAIT_KEY, "5s")) * 1000;
  }
  ...
  public int memoryMapBytes() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("css.storage.memoryMapThreshold", "2m")));
  }
  ...
  public boolean lazyFileDescriptor() {
    return conf.getBoolean(CSS_NETWORK_IO_LAZYFD_KEY, true);
  }
  ...
  public boolean verboseMetrics() {
    return conf.getBoolean(CSS_NETWORK_VERBOSE_METRICS, false);
  }
  ...
  public boolean enableTcpKeepAlive() {
    return conf.getBoolean(CSS_NETWORK_IO_ENABLETCPKEEPALIVE_KEY, false);
  }
  ...
  public int portMaxRetries() {
    return conf.getInt("css.port.maxRetries", 16);
  }
  ...
  public boolean encryptionEnabled() {
    return conf.getBoolean("css.network.crypto.enabled", false);
  }
  ...
  public String cipherTransformation() {
    return conf.get("css.network.crypto.cipher", "AES/CTR/NoPadding");
  }
  ...
  public String keyFactoryAlgorithm() {
    return conf.get("css.network.crypto.keyFactoryAlgorithm", "PBKDF2WithHmacSHA1");
  }
  ...
  public int keyFactoryIterations() {
    return conf.getInt("css.network.crypto.keyFactoryIterations", 1024);
  }
  ...
  public int encryptionKeyLength() {
    return conf.getInt("css.network.crypto.keyLength", 128);
  }
   ...
   */
  public int ivLength() {
    return conf.getInt("css.network.crypto.ivLength", 16);
  }
  ...
  public String keyAlgorithm() {
    return conf.get("css.network.crypto.keyAlgorithm", "AES");
  }
  ...
  public boolean saslFallback() {
    return conf.getBoolean("css.network.crypto.saslFallback", true);
  }
  ...
  public boolean saslEncryption() {
    return conf.getBoolean("css.authenticate.enableSaslEncryption", false);
  }
  ...
  public int maxSaslEncryptedBlockSize() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("css.network.sasl.maxEncryptedBlockSize", "64k")));
  }
  ...
  public boolean saslServerAlwaysEncrypt() {
    return conf.getBoolean("css.network.sasl.serverAlwaysEncrypt", false);
  }
  ...
  public boolean sharedByteBufAllocators() {
    return conf.getBoolean("css.network.sharedByteBufAllocators.enabled", true);
  }
  ...
  public boolean preferDirectBufsForSharedByteBufAllocators() {
    return conf.getBoolean("css.network.io.preferDirectBufs", true);
  }
  ...
  public Properties cryptoConf() {
    return CryptoUtils.toCryptoConf("css.network.crypto.config.", conf.getAll());
  }
   ...
   * retry according to the shuffle retry configs (see `css.shuffle.io.maxRetries` and
   * `css.shuffle.io.retryWait`), if those limits are reached the task will fail with fetch
   * failure.
   */
  public long maxChunksBeingTransferred() {
    return conf.getLong("css.shuffle.maxChunksBeingTransferred", Long.MAX_VALUE);
  }
   ...
   * {@link com.bytedance.css.network.server.TransportServer}'s default EventLoopGroup
  ...
  public int chunkFetchHandlerThreads() {
    if (!this.getModuleName().equalsIgnoreCase("fetch")) {
      return 0;
    }
    int chunkFetchHandlerThreadsPercent =
      Integer.parseInt(conf.get("css.shuffle.server.chunkFetchHandlerThreadsPercent"));
    int threads =
  ...
  public boolean separateChunkFetchRequest() {
    return conf.getInt("css.shuffle.server.chunkFetchHandlerThreadsPercent", 0) > 0;
  }
