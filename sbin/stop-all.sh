...
"${CSS_HOME}/sbin"/stop-workers.sh
"${CSS_HOME}/sbin"/stop-master.sh
...
then
  ...
  while true
  do
    running=`${CSS_HOME}/sbin/workers.sh ps -ef | grep -v grep | grep deploy.worker.Worker`
    if [ -z "$running" ]
    then
